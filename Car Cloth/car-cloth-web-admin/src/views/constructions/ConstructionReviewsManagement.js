...
/* MAIN COMPONENT */
function ConstructionReviewsManagement() {
    ...
    const tableConfigs = [
        ...
        { 'header': '업체​', 'type': AutoTableCellType.Text, 'bindingKey': 'company.companyName' },
        { 'header': '내용', 'type': AutoTableCellType.URL, 'bindingKey': 'id', 'bindingUrlMask': 'content', 'baseURL': CONSTRUCTION_ROUTER.REVIEWS_DETAILS },
    ...
    useEffect(() => {
        ...
        ConstructionService.getConstructionReviews(paging, filterValue).then((res) => {
    ...
    function updateExposures(items) {
        ...
        ConstructionService.updateReviewStatus(items).then((res) => {

            if (res.status === 200) {
                window.location.href = CONSTRUCTION_ROUTER.REVIEWS;
            }
        });
    }
    ...
    return (
        <CContainer fluid>
            <CHeader>
                <CHeaderBrand className={'fw-bold'}>시공 후기 목록</CHeaderBrand>
            </CHeader>
...
export default ConstructionReviewsManagement;
