...
const CategoryRegister = ({ show, setShow, callback }) => {
    ...
    const submit = () => {
        if (!source.title || !source.icon) {
            ...
            CategoryService.register({ title: source.title, icon: { fileName: files[0].name } })
                .then((res) => {
                    StorageService.upload(res.data.objectKey, files[0])
                    .then((r) => {
                        handler({ title: "", icon: null })
        ...
        <>
            ...
            >
                <CModalHeader className="bg-dark">
                    <h5 className="text-white">카테고리 등록​</h5>
                </CModalHeader>
                <CModalBody>
